...
class Report:
    ...
    def get_members_not_reporting_more_than_7days(self, id_group):
        ...
        try:
            query = text("""
                         ...
                         FROM member_table m
                                  JOIN (SELECT r.id_member,
                                               MAX(r.date)        AS last_report_date,
                                               MAX(rd.id_chapter) AS last_read_chapter_id
                                        FROM report_table r
                                                 JOIN report_details_table rd ON r.id_report = rd.id_report
                                        GROUP BY r.id_member
                                        HAVING last_report_date <= CURDATE() - INTERVAL 7 DAY) last_read
                                       ON m.id_member = last_read.id_member
    ...
    def get_reading_percentage(self, id_group):
        ...
        try:
            query = text("""
                         SELECT (MAX(rd.id_chapter) / 1189) * 100 AS group_reading_percentage
    ...
    def get_members_segmentation(self, id_group):
        ...
        try:
            ...
            query_ontime = text("""
                                WITH LatestReport AS (SELECT id_member, MAX(date) AS latest_date
                                                      FROM report_table
                                                      GROUP BY id_member),
                                     MemberLatestReport AS (SELECT r.id_report, r.id_member, r.date
                                                            FROM report_table r
                                                                     JOIN LatestReport lr
                                                                          ON r.id_member = lr.id_member AND r.date = lr.latest_date),
                                     MaxChapterPerMember
                                         AS (SELECT mlr.id_member, MAX(rd.id_chapter) AS latest_id_chapter
